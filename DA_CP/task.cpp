...
void LoadNodes(const std::string& nodesFileName, std::vector<Node> &Nodes){
    ...
    Node currentNode;
    while(nodesFile >> currentNode.id >> currentNode.latitude >> currentNode.longitude){
...
double Distance(Node v, Node u){
    if (std::isnan(acos(sin(PI / 180 * (v.latitude)) * sin(PI / 180 * (u.latitude)) +
                        cos(PI / 180 * (v.latitude)) * cos(PI / 180 * (u.latitude)) *
                        cos(PI / 180 * (u.longitude - v.longitude))))){
        return 0;
    } else {
    return R * acos(sin(PI / 180 * (v.latitude)) * sin(PI / 180 * (u.latitude)) +
                    cos(PI / 180 * (v.latitude)) * cos(PI / 180 * (u.latitude)) *
                    cos(PI / 180 * (u.longitude - v.longitude)));
    }
}
...
void AddNeighbors(std::ifstream &graphFile, Candidate activeCandidate, std::set<Candidate> &openList){
    ...
    graphFile.seekg(sizeof(size_t) + activeCandidate.index * (2 * sizeof(uint32_t) + 2 * sizeof(double)) + sizeof(uint32_t), std::ios_base::beg);
    graphFile.read((char*)&activeNode.latitude, sizeof(double));
    graphFile.read((char*)&activeNode.longitude, sizeof(double));
    ...
    graphFile.seekg(sizeof(size_t) + ids.size() * (2 * sizeof(uint32_t) + 2 * sizeof(double)) + sizeof(uint32_t) + shifts[activeIndex] * sizeof(uint32_t), std::ios_base::beg);
    ...
    for (int i = 0; i < neighbors.size(); ++i){
        ...
        graphFile.seekg(sizeof(size_t) + neighborIndex * (2 * sizeof(uint32_t) + 2 * sizeof(double)) + sizeof(uint32_t), std::ios_base::beg);
        graphFile.read((char*)&neighbors[i].latitude, sizeof(double));
        graphFile.read((char*)&neighbors[i].longitude, sizeof(double));
...
void AStar(uint32_t startId, uint32_t finishId, size_t numberOfNodes, std::ifstream &graphFile, double &ans, std::vector<uint32_t> &path){
    ...
    graphFile.seekg(sizeof(size_t) + startIndex * (sizeof(uint32_t) + 2 * sizeof(double) + sizeof(uint32_t)) + sizeof(uint32_t), std::ios_base::beg);
    graphFile.read((char*)&start.latitude, sizeof(double));
    graphFile.read((char*)&start.longitude, sizeof(double));
    ...
    graphFile.seekg(sizeof(size_t) + finishIndex * (sizeof(uint32_t) + 2 * sizeof(double) + sizeof(uint32_t)) + sizeof(uint32_t), std::ios_base::beg);
    graphFile.read((char*)&finish.latitude, sizeof(double));
    graphFile.read((char*)&finish.longitude, sizeof(double));
...
int main(int argc, char* argv[]) {
    if (strcmp(argv[1],"preprocess") == 0){
        ...
        for (uint32_t i = 0; i < Nodes.size(); ++i){
            ...
            graphFile.write(nodeLatitude, sizeof(double));
            graphFile.write(nodelongitude, sizeof(double));
    ...
    if (strcmp(argv[1], "search") == 0){
        ...
        for (uint32_t i = 0; i < numberOfNodes; ++i){
            ...
            graphFile.seekg(2 * sizeof(double), std::ios_base::cur);
